...
# Calculate vigorous, moderate, and walking time (in minutes)
...
pes <- pes %>%
  mutate(
    TL_Vig_AP = coalesce(q2_1b_1, 0) * 60 + coalesce(q2_1b_2, 0),
    TL_Mod_AP = coalesce(q2_2b_1, 0) * 60 + coalesce(q2_2b_2, 0),
    TL_Walking = coalesce(q2_3b_1, 0) * 60 + coalesce(q2_3b_2, 0)
  )

# --- Exclusion criteria (> 960) ---
pes <- pes %>%
  mutate(total_activity_min = TL_Walking + TL_Mod_AP + TL_Vig_AP)

excluded_n <- sum(pes$total_activity_min > 960, na.rm = TRUE)

pes <- pes %>%
  filter(total_activity_min <= 960)

cat("Excluded cases exceeding 960 min/day:", excluded_n, "\n")
cat("Remaining sample size:", nrow(pes), "\n")
...
pes <- pes %>%
  mutate(sitting = coalesce(q2_4_1, 0) * 60 + coalesce(q2_4_2, 0))

# Total MVPA
pes <- pes %>%
  mutate(mod_vig_PA = moderate_PA + vigorous_PA + walking)
...
semPaths(fit,
         what = "std",           # 標準化係数を表示
         layout = "tree",        # レイアウト（tree, circle, spring等）
         edge.label.cex = 1.0,   # ラベルサイズ
         rotation = 2)           # 回転（1=0度、2=90度、3=180度、4=270度）
